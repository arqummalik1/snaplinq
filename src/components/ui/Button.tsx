...
export const Button = ({
    ...
    const variants = {
        ...
        secondary:
            'bg-white/80 dark:bg-slate-800/80 border border-white/40 dark:border-slate-700/40 backdrop-blur-xl shadow-sm',
    ...
    const content = (
        <View className="flex-row items-center justify-center gap-2">
            {loading ? (
                <ActivityIndicator
                    size="small"
                    color={variant === 'primary' ? 'white' : variant === 'danger' ? '#ef4444' : '#64748b'}
                />
